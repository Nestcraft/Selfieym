...
class User extends BaseUser
{
    ...
    */
	...
	public static function countInfluencerByParentCategories()
	{
		$sql1 = 'SELECT c.id as id, count(*) as total' . '
            FROM ' . DBTool::table('social') . ' as a
            INNER JOIN ' . DBTool::table('categories') . ' as c ON c.id=a.category_id AND c.active=1
            INNER JOIN ' . DBTool::table('users') . ' as u ON u.id=a.user_id
            WHERE u.country_code = :countryCode AND (u.user_type_id=2 AND u.verified_email=1 AND u.verified_phone=1) AND u.deleted_at IS NULL
            GROUP BY c.id';
