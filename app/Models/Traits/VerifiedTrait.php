...
// IS FEATURED ADDITION WEBC (20 die("one"))

public function getIsFeaturedHtml()
{
    if (!isset($this->is_featured)) return false;

        // Get checkbox
    $out = ajaxCheckboxDisplay($this->{$this->primaryKey}, $this->getTable(), 'is_featured', $this->is_featured);

        // Get all entity's data
    $entity = self::find($this->{$this->primaryKey});
    

    if (empty($entity->is_featured)) {
        if ($entity->is_featured != 1) {
                // ToolTip
            $toolTip = (!empty($entity->is_featured)) ? 'data-toggle="tooltip" title="' . trans('admin::messages.To') . ': ' . $entity->is_featured . '"' : '';
        ...
        $out = checkboxDisplay($this->is_featured);
    }
