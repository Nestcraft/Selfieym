...
class CreateController extends FrontController
{
	...
	public function postForm(PostRequest $request, $tmpToken = null)
	{
		...
		$post->required_influencer  = $request->input('required_influencer');
		$post->influencer_age  = $request->input('influencer_age');
		$post->influencer_gender  = $request->input('influencer_gender');
		if($request->input('instagram_chk') == 1){
			$post->instagram_followers  = $request->input('instagram_followers').$request->input('instagram_unit');
		}
		if($request->input('facebook_chk') == 1){
			$post->facebook_likes  = $request->input('facebook_likes').$request->input('facebook_unit');
		}
		if($request->input('twitter_chk') == 1){
			$post->twitter_followers  = $request->input('twitter_followers').$request->input('twitter_unit');
		}
		if($request->input('youtube_chk') == 1){
			$post->youtube_subscribers  = $request->input('youtube_subscribers').$request->input('youtube_unit');
		}
		if($request->input('tiktok_chk') == 1){
			$post->tiktok_likes  = $request->input('tiktok_likes').$request->input('tiktok_unti');
		}
		if($request->input('quora_chk') == 1){
			$post->quora_followers  = $request->input('quora_followers').$request->input('quora_unit');
		}
		if($request->input('web_chk') == 1){
			$post->web_followers  = $request->input('web_followers').$request->input('web_unit');
		}
		...
		// Save the Company's document
		if ($request->hasFile('uploadfile')) {
			//$post->document = $request->file('uploadfile');
			$post->document = $this->fileUpload($request,'uploadfile');
		}
		// Save
		$post->save();
	...
	public function fileUpload($request,$attrname)
	{
		$imageName = time().$attrname.'.'.$request->$attrname->extension();  
		$request->$attrname->move(public_path('document'), $imageName);
