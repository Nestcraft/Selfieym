...
<body class="hold-transition {{ config('larapen.admin.skin') }} sidebar-mini">
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<p><b>Total Employer:<?php echo $total_employers;?>&nbsp;&nbsp; &nbsp;Total Employer Wallet Balance:₹ <?php echo $emp_wallet_balance;?> &nbsp;&nbsp;&nbsp;Total Influencer:<?php echo $total_influencer;?> &nbsp; &nbsp; &nbsp;Total Influencer Wallet Balance:₹ <?php echo $influencer_wallet_balance;?>&nbsp; &nbsp; &nbsp;Commission Earned:₹ <?php echo $commisstion; ?> </b></p>
			<!-- <p>Total Influencer:<?php echo $total_influencer;?> Total Influencer Wallet Balance:<?php echo $influencer_wallet_balance;?> Commission Earned:<?php ?></p> -->
				...
				<table class="table table-bordered table-striped display dt-responsive nowrap dataTable dtr-inline" id="table">
					<thead>
						<tr>
							...
							<!-- <th class="text-center">User Bank Detils</th> -->
						</tr>
					</thead>
					<tbody>
						<?php
...
foreach ($list_users as $users): ?>
							<tr>
								<td><?=$count;?></td>
								<td><?=$users->name;?></td>
								<?php if ($users->user_type_id == 1 && $users->id != 1): ?>
									<td><?="Employer";?></td>
									<?php elseif ($users->user_type_id == 2): ?>
									<td><?="Job Seeker";?></td>
									<?php else: ?>
									<td><?="Administrator";?></td>
									<?php endif;?>
									...
									<td><?php if(isset($users->wallet_amount)):?>₹<?= $users->wallet_amount; ?>
                                    <?php else:?>
                                    	<?= '₹0';?>
									<?php endif; ?></td>

										<td><a href="/admin/wallet/<?=$users->id;?>" class="btn btn-xs btn-primary">Debit / Credit</a>&nbsp;<a href="/admin/usertransactions/<?=$users->id;?>" class="btn btn-xs btn-primary" >View Transaction</a></td>
										<!-- <td><a href="/admin/wallet/<?=$users->id;?>" class="btn btn-xs btn-primary">Debit / Credit</a>&nbsp;<a href="/admin/usertransactions/<?=$users->id;?>" class="btn btn-xs btn-primary" >View Transaction</a></td> -->
									</tr>
									<?php $count++;?>
								<?php endforeach;?>
