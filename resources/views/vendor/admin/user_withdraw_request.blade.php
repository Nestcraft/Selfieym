...
<body class="hold-transition {{ config('larapen.admin.skin') }} sidebar-mini">
					...
					<tbody>
						...
						foreach($withdraw_request as $users): ?>
							<tr>
								...
								<td><b><?php echo ucfirst($users->front_payment_method); 
								echo "</b>";
								$span = "";
								...
								?>
								<div style="display: none;" id="extrainfo_<?php echo $users->withdraw_request_id;?>">
									<?php echo $span;?>
								</div>
								</td>
								<td> <?= $users->created_date; ?></td>
								<td><a href="javascript:void(0)" data-request-id = '<?= $users->withdraw_request_id;?>'data-enc-id = '<?= $users->user_id;?>' data-amount = "<?= $users->amount;?>" class="btn btn-xs btn-success approve">Approve</a>&nbsp;<a data-request-id = '<?= $users->withdraw_request_id;?>'data-enc-id = '<?= $users->user_id;?>'href="javascript:void(0)" data-amount = "<?= $users->amount;?>" class="btn btn-xs btn-danger reject">Reject</a></td>
							</tr>
							<?php $count++; ?>
						<?php endforeach; ?>
					</tbody>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<h4 style="text-align:center;" class="requested_details"></h4>
	...
	<script>
		$(document).ready(function()
		{
			...
			var requested_details = $("#extrainfo_"+request_id).html();
				$(".requested_details").html(requested_details);
