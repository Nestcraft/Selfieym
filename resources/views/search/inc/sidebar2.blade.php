				...
				<div class="filter-salary filter-content ">
					<form role="form" class="form-inline" action="{{ $fullUrlNoParams }}" method="GET">
						...
						<div class="form-group col-sm-4 no-padding">
							<input type="text" id="minSalary" name="minSalary" value="{{ request()->get('minSalary') }}" placeholder="2000" class="form-control">
						</div>
					...
					<ul class="list-unstyled">
						<li>
							@if ($cats->has($parentId))
								...
								<a href="{{ \App\Helpers\UrlGen::influencer($cats->get($parentId)) }}" title="{{ $cats->get($parentId)->name }}">
									...
									@foreach ($cats->groupBy('parent_id')->get($parentId) as $iSubCat)
										...
										<li>
											@if ((isset($uriPathSubCatSlug) and $uriPathSubCatSlug == $iSubCat->slug) or (request()->input('sc') == $iSubCat->tid))
												<strong>
													<a href="{{ \App\Helpers\UrlGen::influencer($iSubCat, 1) }}" title="{{ $iSubCat->name }}">
			...
			<div id="catsList" class="categories-list list-filter" <?php echo (isset($style)) ? $style : ''; ?>>
				...
				<ul class="list-unstyled">
					@if($cats->groupBy('parent_id')->has(0))
						...
						<a href="{{ \App\Helpers\UrlGen::influencer($iCat) }}" title="{{ $iCat->name }}">
											<span class="title">{{ $iCat->name }}</span>
											<span class="count">&nbsp;{{ $countInfluencer->get($iCat->tid)->total ?? 0 }}</span>
										</a>
									</strong>
								@else
									<a href="{{ \App\Helpers\UrlGen::influencer($iCat) }}" title="{{ $iCat->name }}">
					...
					@if (isset($cities) and $cities->count() > 0)
						@foreach ($cities as $city)
							<?php
								...
								$fullUrlLocation = lurl(trans('routes.i-search', $attr), $attr);
								$locationParams = [
									'l'  => $city->id,
									'r'  => '',
									'c'  => (isset($cat)) ? $cat->tid : '',
